...
public class UpgradeButtonHandler : MonoBehaviour
{
    ...
    bool quantityMet(BuildingUpgrade upgrade)
    {
        ...
        string theThing = new Regex(" flowers").Replace(upgrade.quantityNeeded, "");
        bool b = upgrade.upgradeClass == "flavored flowers" && theThing.Length < 20 && gameStats.flowers >= Convert.ToInt64(theThing);

        theThing = upgrade.quantityNeeded.Replace(" hand-made flowers", "");
        bool c = upgrade.upgradeClass == "clicking upgrades" && theThing.Length < 20 && gameStats.handmadeFlowers >= Convert.ToInt64(theThing);
    ...
    void Update()
    {
        ...
        for (int i = 0; i < upgradeButtons.Length; i++)
        {
            ...
            if (upgradesQuantityMet.Count > 0)
            {
                ...
                popupTexts[i].text = upgradeButtons[i].upgrade.name + " (" + gameStats.formatNumber(decimal.Parse(upgradeButtons[i].upgrade.basePrice), 0) + " flowers)\n" + upgradeButtons[i].upgrade.description;
                ...
                if (gameStats.flowers >= Convert.ToInt64(upgradeButtons[i].upgrade.basePrice))
                {
    ...
    void updateCursorUpgradeIfNeeded(BuildingUpgrade upgrade)
    {
        if (upgrade.enabled && upgrade.upgradeType == "cursor" && !(upgrade.name == "Reinforced index finger" || upgrade.name == "Carpal tunnel prevention cream" || upgrade.name == "Ambidextrous"))
        {
            ...
            if (totalNonCursors != oldTotalNonCursors)
            {
                ...
                string flowerGainPerNonCursorString = new Regex("(?<=\\+)(\\d|\\.)+(?= flowers)").Match(description).Groups[0].ToString();

                double flowerGainPerNonCursor = Convert.ToDouble(flowerGainPerNonCursorString);

                double flowersGained = flowerGainPerNonCursor * totalNonCursors;

                gameStats.flowersPerClickAddOn -= (decimal)upgrade.flowersPerSecondAddOn;
                buildingButtonHandler.findButtonWithName("cursor").flowersPerSecondAddOn -= upgrade.flowersPerSecondAddOn;

                upgrade.flowersPerSecondAddOn = flowersGained;
    ...
    void updateGrandmaTypeUpgradeIfNeeded(BuildingUpgrade upgrade)
    {
        if (upgrade.enabled && upgrade.upgradeType == "grandma types")
        {
            ...
            theBuilding.flowersPerSecondAddOn = theBuilding.flowersPerSecond * theBuilding.flowersPerSecondMultiplier * 0.01 * totalGrandmas / grandmasPerPercentCpsIncrease;
        }
    }

    // yeah it's kind of odd to use the upgrade's "flowers per second add on" as a per-click add but whatever it works
    void updateClickingUpgradeIfNeeded(BuildingUpgrade upgrade)
    {
        if (upgrade.enabled && upgrade.upgradeType == "clicking upgrades")
        {
            double flowersPerSecond = (double)gameStats.flowersPerSecondTotal;
            //			decimal flowersPerClick = gameStats.flowersPerClick;
            double add = 0.01 * flowersPerSecond;
            gameStats.flowersPerClickAddOn -= (decimal)upgrade.flowersPerSecondAddOn;
            upgrade.flowersPerSecondAddOn = add;
            gameStats.flowersPerClickAddOn += (decimal)upgrade.flowersPerSecondAddOn;
        }
    }
    ...
    {
        if (Convert.ToInt64(button.upgrade.basePrice) <= gameStats.flowers && !button.upgrade.enabled && quantityMet(button.upgrade))
        {
            button.upgrade.enabled = true;
            gameStats.flowers -= Convert.ToInt64(button.upgrade.basePrice);
            ...
            {
                if (button.upgrade.upgradeType == "flavored flowers")
                {
                    ...
                    gameStats.flowersPerSecondMultiplier *= 1.0m + decimal.Parse(multiplierIncrease) / 100.0m;
                }
                else
                {
                    buildingButtonHandler.findButtonWithName(button.upgrade.upgradeType == "grandma types" ? "grandma" : button.upgrade.upgradeType).flowersPerSecondMultiplier *= 2;
                }
                if (button.upgrade.upgradeType == "cursor")
                    gameStats.flowersPerClickMultiplier *= 2;
            }
        }
    }
