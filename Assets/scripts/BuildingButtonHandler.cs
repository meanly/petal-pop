...
public class BuildingButtonHandler : MonoBehaviour
{
    ...
    double[] baseFlowersPerSeconds = new double[] { 0.1, 1.0, 8.0, 47.0, 260.0, 1400.0, 7800.0, 44000.0 };
    ...
    void addNewButton(float x, float y)
    {
        ...
        createButton(names[lastIndex], prices[lastIndex], baseFlowersPerSeconds[lastIndex]);
    ...
    void Update()
    {
        for (int i = 0; i < buildingButtonLabelNames.Count; i++)
        {
            ...
            if (gameStats.flowers >= currentButton.price && !currentButton.isNameVisible)
            {
            ...
            if (currentButton.isNameVisible)
            {
                ...
                popupTexts[i].text = currentButton.myName + " (" + gameStats.formatNumber(currentButton.price, 0) + " flowers)\n" + (currentButton.count == 0 ? "" : currentButton.getProductionDescriptionSingle() + "\n") + currentButton.getProductionDescriptionTotal();
                popupTexts[i].rectTransform.anchoredPosition = new Vector2(0f, 0f);
            }
            else
            {
                buildingButtonLabelNames[i].text = "???";
                popupTexts[i].text = "??? (" + gameStats.formatNumber(currentButton.price, 0) + " flowers)";
            }
            buildingButtonLabelPrices[i].text = gameStats.formatNumber(currentButton.price, 0) + " flowers";


            buildingLevelTexts.ElementAt(i).text = buildingButtons[i].count == 0 ? "" : buildingButtons[i].count + "";

            if (gameStats.flowers >= buildingButtons.ElementAt(i).price)
            {
    ...
    void createButton(string newName, int newPrice, double newCPS)
    {
        ...
        button.flowersPerSecond = newCPS;
        button.flowersPerSecondMultiplier = 1.0;
    }
    ...
    {
        if (gameStats.flowers >= button.price)
        {
            gameStats.flowersPerSecond += (decimal)button.flowersPerSecond;
            gameStats.flowers -= button.price;
            button.count++;
    ...
    public void refreshFlowersPerSecond()
    {
        double flowersPerSecond = 0.0;
        for (int i = 0; i < buildingButtons.Count; i++)
            flowersPerSecond += buildingButtons[i].getTotalFlowersPerSecondCombined();
        gameStats.flowersPerSecond = (decimal)flowersPerSecond;
    }
